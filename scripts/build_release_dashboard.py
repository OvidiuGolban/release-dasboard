...
Components:
    Only tooling releases whose tag looks like "<tla>-<n>" (e.g. "hlpbf-2079") are
    shown by default; each such tag maps to its exact TLA (hlpbf, hlppp, hlpsbg,
    crpbf, ...). Legacy numeric tags (e.g. "1764") cannot be attributed to a
    specific sub-TLA and are hidden unless DASHBOARD_INCLUDE_LEGACY is set.
...
# ("v1.2.3") do not match and are treated as legacy/unattributable.
...
def infer_repo_component(releases, repo):
    """Repo-level fallback component (used only for legacy tags when included).

    Uses the single tooling tag prefix when the repo has exactly one; otherwise
    the repo name.
    """
    prefixes = set()
    for rel in releases:
...
def collect_rows(repos):
    ...
    # By default only tooling releases ("<tla>-<n>") are shown, because their tag
    # carries the exact TLA/component (hlpbf, hlppp, hlpsbg, ...). Legacy numeric
    # tags (e.g. "1764") cannot be attributed to a specific sub-TLA; set
    # DASHBOARD_INCLUDE_LEGACY=1 to include them (grouped under the repo).
    include_legacy = os.environ.get("DASHBOARD_INCLUDE_LEGACY", "").strip().lower() \
        in ("1", "true", "yes")
    rows = []
    for repo in repos:
        ...
        for rel in releases:
            ...
            if prefix is None and not include_legacy:
                continue
